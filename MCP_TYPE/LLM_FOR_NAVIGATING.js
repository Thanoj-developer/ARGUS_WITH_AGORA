...
// System Prompt placeholder (User: "be ready with the system prompt i will give later")
...
function getSystemPrompt() {
  try {
    ...
    console.error('[Auto Nav LLM] Failed to load system prompt:', err);
  }
  // Default system prompt until user provides the custom one
  return `You are the Auto Navigation agent. Based on the user instruction and page accessibility tree, output the next browser action as a JSON object:
{
  "index": <number>,
  "action": "click" | "fill",
  "value": "<text_if_fill>"
...
/**
 * Streams the auto-navigation LLM completion.
 * Useful for real-time console streaming.
 */
async function streamNavigationAction(query, accessibilityTree, historyStr = '', onChunk) {
  ...
  for (const model of models) {
    try {
      console.log(`[Auto Nav LLM] Querying model stream ${model}...`);
      ...
      console.warn(`[Auto Nav LLM] Model stream ${model} failed:`, err.message);
      lastError = err;
    }
  }
...
/**
 * Non-streaming helper to get the navigation action directly.
 */
...
${JSON.stringify(personalMemory, null, 2)}`;
  ...
  for (const model of models) {
    try {
      console.log(`[Auto Nav LLM] Querying model ${model}...`);
      const completion = await openai.chat.completions.create({
        model,
        messages,
        temperature: 0.2,
      ...
      console.warn(`[Auto Nav LLM] Model ${model} failed:`, err.message);
      lastError = err;
    }
  }
